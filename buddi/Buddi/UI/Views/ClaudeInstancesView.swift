...
struct ClaudeInstancesView: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            HStack {
                Label("Clicky agents", systemImage: "cursorarrow.click.2")
                ...
                Button {
                    ClickySessionAdapter.shared.spawnAgent()
                } label: {
    ...
    private var emptyState: some View {
        VStack(spacing: 8) {
            Text("No Clicky agents")
                .font(.subheadline.weight(.medium))
                .foregroundStyle(.secondary)

            Text("Spawn one from the notch")
                .font(.caption)
...
struct InstanceRow: View {
    ...
    private let clickyBlue = Color(red: 0.20, green: 0.78, blue: 1.00)
    ...
    private var isClickySession: Bool {
        ClickySessionAdapter.shared.isClickySession(session.sessionId)
    }

    private var clickyAgentColor: Color {
    ...
    private var stateIndicator: some View {
        if isClickySession {
            ZStack {
                Circle()
                    .stroke(clickyAgentColor.opacity(0.85), lineWidth: 1.5)
                ...
                Image("steve")
                    ...
                    .clipShape(Circle())
                    .shadow(color: clickyAgentColor.opacity(0.45), radius: session.phase == .processing ? 8 : 2)
            }
            .rotationEffect(.degrees(session.phase == .processing ? Double(spinnerPhase * 12) : 0))
