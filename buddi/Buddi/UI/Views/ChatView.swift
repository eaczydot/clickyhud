...
struct ChatView: View {
    ...
    private var canSendMessages: Bool {
        ClickySessionAdapter.shared.isClickySession(session.sessionId) || (session.isInMultiplexer && session.tty != nil)
    }

    private var inputBar: some View {
        HStack(spacing: 10) {
            TextField(canSendMessages ? "Message Clicky..." : "Clicky output is shown here for this agent", text: $inputText)
    ...
    private func sendToSession(_ text: String) async {
        ...
        if await ClickySessionAdapter.shared.isClickySession(session.sessionId) {
            await ClickySessionAdapter.shared.sendMessage(text, to: session.sessionId)
            return
        }
...
struct NewMessagesIndicator: View {
    ...
    var body: some View {
        Button(action: onTap) {
            ...
            .background(
                Capsule()
                    .fill(Color(red: 0.20, green: 0.78, blue: 1.00))
