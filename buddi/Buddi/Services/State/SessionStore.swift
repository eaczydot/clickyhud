...
actor SessionStore {
    ...
    nonisolated static let logger = os.Logger(subsystem: "com.splab.buddi", category: "Session")
    ...
    private func processHookEvent(_ event: HookEvent) async {
        ...
        if let pid = event.pid {
            ...
            session.isInCmux = ProcessTreeBuilder.shared.isInCmux(pid: pid, tree: tree)
        }
        if let ws = event.cmuxWorkspace, let surf = event.cmuxSurface {
            session.cmuxWorkspaceRef = ws
            session.cmuxSurfaceRef = surf
        }
    ...
    func upsertClickyAgentSession(
        ...
        session.chatItems = chatItems
        session.conversationInfo = ConversationInfo(
            summary: agentName,
            lastMessage: lastAgentOutput,
            lastMessageRole: "assistant",
            lastToolName: nil,
            firstUserMessage: lastUserMessage,
            lastUserMessageDate: lastUserMessage == nil ? nil : Date()
        )
        session.lastActivity = Date()
        sessions[sessionId] = session
        publishState()
    ...
    func removeClickyAgentSession(sessionId: String) {
        sessions.removeValue(forKey: sessionId)
        publishState()
    }
