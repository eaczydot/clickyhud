...
actor TmuxSessionMatcher {
    ...
    private func capturePaneContent(tmuxPath: String, target: TmuxTarget) async -> String? {
        ...
        case .tmux:
            return await captureTmuxPane(tmuxPath: tmuxPath, target: target)
        case .cmux:
            return await captureCmuxPane(target: target)
        }
    }

    private func captureTmuxPane(tmuxPath: String, target: TmuxTarget) async -> String? {
        do {
    ...
    private func captureCmuxPane(target: TmuxTarget) async -> String? {
        guard let cmuxPath = await TmuxPathFinder.shared.getCmuxPath(),
              let workspace = target.cmuxWorkspaceRef,
              let surface = target.cmuxSurfaceRef else {
            return nil
        }

        do {
            let output = try await ProcessExecutor.shared.run(cmuxPath, arguments: [
                "--json", "read-screen", "--workspace", workspace,
                "--surface", surface, "--scrollback", "--lines", "500"
            ])
